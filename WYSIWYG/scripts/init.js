(function($){
  ...
  function init(){
   ...
   if(mw.util.$content && mw.util.$content.text().indexOf( '__NORICHEDITOR__' ) > -1){
      return;
    }
  ...
  function toggleEditor(toggle, wikieditor, toolbar){
    ...
    // Show/Hide also WikiEditor
    if (mw.config.get('wgCKeditorVisible') == true)  {
  ...
  $(document).ready( function(){
    ...
    CKEDITOR.on('instanceReady', function(event){
      ...
      if (mw.user.options.get('riched_use_toggle')) {
        $('#ckTools').remove();
        if ($('#ckTools').length == 0)  {
            $("<div id='ckTools' style='float: right; display: block;'> [ <a id='toggleAnchor' class='fckToogle' href=''>Show WikiTextEditor</a> ]</div>").insertBefore("#editform");
        }
      }
      // Move & hide some elements
      $('#wysiwyg-status-msg').insertBefore('#ckTools');
      $('#wysiwyg-status-msg').css('float', 'left');
      $('#wpPreview').hide();
      $('#wpDiff').hide();
      // ---------------------------------------------
